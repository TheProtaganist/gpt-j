...
class GPTJ_API(GPTJ):
    ...
    def imaginary_friend(self, user_id, friend):
        self.ID = f"{str(user_id)}: "
        self.your_friend = f"{str(friend)}: "
        ...
        self.formatted_convo = [i for e in zip(self.user_aspect, list(self.bot_aspect) * len(self.user_aspect)) for i in e][: self.length]
